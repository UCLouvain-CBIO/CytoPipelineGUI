    ...
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        conditionalPanel(
          ...
          ns = NS(id),
          #condition = paste0("input.", NS(id, "transfoType"), " == 'linear'"),
          numericInput(inputId = NS(id, "linA"),
                       label = "slope",
                       value = 1., min = 0., max = 1000000, step = NA),
          
          numericInput(inputId = NS(id, "linB"),
                       label = "intercept",
                       value = 0., min = -Inf, max = Inf, step = NA)),
        
        # Input: set of inputs for conditional transformations
        conditionalPanel(
          condition = "input.transfoType == 'logicle'",
          ns = NS(id),
          #condition = paste0("input.", NS(id, "transfoType"), " == 'logicle'"),
          sliderInput(inputId = NS(id, "negDecades"),
          ...
          sliderInput(inputId = NS(id, "width"),
                      label = "Width:",
                      min = 0,
                      max = 3,
                      step = 0.1,
          ...
          sliderInput(inputId = NS(id, "posDecades"),
                      label = "Pos Decades:",
                      min = 2,
                      max = 7,
                      step = 0.1,
        ...
        conditionalPanel(
          ...
          actionButton(inputId = NS(id, "apply"),
                       label = "Apply to channel")),
      
        conditionalPanel(
          condition = tolower(as.character(enableTransfoListSave)),
          downloadButton(outputId = NS(id, "save"),
                         label = "Save")),
        
        # conditionalPanel(
        #   condition = tolower(as.character(transListSource == "input")),
...
scaleTransformServer <- function(id, path, transfoList = NULL, ff = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    if (!is.null(transfoList)) {
      if (is.null(ff)) {
        stop("if transfoList parameter is passed as non null, so must be ff parameter")
      } 
    } else {
      if (!is.null(ff)) {
        stop("if ff parameter is passed as non null, so must be transfoList parameter")
      }
    }
    ...
    currentFF <- reactive({
      ...
      if (is.null(currentFFFull)) {
        ...
        channel = channelLabel2Name(input$transfoChannel, currentFFFull())
    ...
    statusMessage <- function(){
      if (!is.null(currentTransfoList)) {
        channels <- c("Comp-450/50Violet-A",
                      "Comp-525/50Violet-A",
                      "Comp-540/30Violet-A",
                      "Comp-585/15Violet-A")
        markers <- c("TETaGC",
                     ...
                     "CD8a")
        message()
        message("In currentTransfoList:")
        message("**********************")
        mapply(FUN = function(x,y){
          message(x,"/",y)
          message(getTransfoParams(currentTransfoList, x))
        }, channels, markers, SIMPLIFY=FALSE)
        
        message("In reactiveVals:")
        message("****************")
        message("r$negDecades: ", round(r$negDecades, 4))
        message("r$width: ", round(r$width, 4))
        message("r$posDecades: ", round(r$posDecades, 4))
        message()
      }
      
    }
    
    getTransfoList <- function(experimentName,
                               path,
                               transfoName) {
      ...
      if (transfoName != " ") {
        ...
        transfoList <-
          getCytoPipelineScaleTransform(pipL,
                                        whichQueue = "scale transform",
      ...
      if (!is.null(transfoList) && !is.null(ff)) {
        ...
        if (!is.null(tPars)) {
          ...
          if (tPars$type == "logicle") {
            ...
            updateSliderInput(inputId = "negDecades",
                              value = val_negDecades)
            updateSliderInput(inputId = "width",
                              value = val_width)
            updateSliderInput(inputId = "posDecades",
                              value = val_posDecades)
            
            synchronizeReactiveTransfoParams(transfoType = tPars$type,
            ...
            updateNumericInput(inputId = "linA",
                               value = val_a)
            updateNumericInput(inputId = "linB",
                               value = val_b)
    ...
    synchronizeReactiveTransfoParams <- function(transfoType,
                                                 linA = 0.,
                                                 linB = 0.,
                                                 negDecades = 0.,
                                                 width = 0.,
    ...
    observeEvent(input$scaleTransfoList, {
      message(paste0("obs event: scaleTransfoList"))
      #browser()
     
      currentTransfoList <<- 
        getTransfoList(input$experimentTransfo,
                       path = path,
                       transfoName = input$scaleTransfoList)
      
      updateTransfoParams(currentTransfoList,
                          currentFF())    
      message(paste0("end obs event: scaleTransfoList"))
    })
    
    observeEvent(input$transfoChannel, {
      message(paste0("obs event: transfoChannel"))
      #statusMessage()
      #browser()
      updateTransfoParams(currentTransfoList,
                          currentFF())
      #message(paste0("end obs event: transfoChannel"))
      #statusMessage()
    })
    ...
    observeEvent({
      ...
      #browser()
      synchronizeReactiveTransfoParams(transfoType = req(input$transfoType),
                                       linA = req(input$linA),
                                       linB = req(input$linB),
    ...
    observeEvent(input$apply, {
      ...
      if (!is.null(currentTransfoList) && !is.null(currentFF())) {
        ...
        if (r$transfoType == "linear") {
          newTrans <- flowCore::linearTransform(a = myList$linA,
                                                b = myList$linB)
        } else {
          newTrans <- flowCore::logicleTransform(w = myList$width,
                                                 m = myList$posDecades + myList$width,
                                                 a = myList$negDecades)
        }
        ...
        currentTransfoList <<- localTransfoList
      }

      
      #message("end obs event: apply")
      #statusMessage()
    ...
    #   if (input$ok) {
    #     message("ok event!")
    #     stopApp(currentTransfoList)}
    # })
    
    output$distPlot <- renderPlot({
      #browser()
      channel <- flowCore::colnames(currentFF())[1]
      plotScaleTransformedChannel(ff = currentFF(),
                                   channel = channel,
